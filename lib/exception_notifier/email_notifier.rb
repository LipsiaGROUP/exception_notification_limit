			...
			def self.extended(base)
				...
				# counting send email
					...
					def send_log_to_tracker
						if  @options[:api_url].present? && File.exist?(@file_path_json)
							url = URI.parse(@options[:api_url])
							File.open(@file_path_json) do |json_file|
								req = Net::HTTP::Post::Multipart.new url.path, 
									name_project: ENV["NAME_PROJECT"],
									file: UploadIO.new(json_file, "application/json", @file_name_json),
									title: compose_subject,
